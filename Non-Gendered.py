...
with open(sys.argv[1]) as myFile:
    ...
    lines = [(eng, spa, [tag.strip().rstrip().replace(' ','_') for tag in tags.split(';')])
             for eng, spa, tags, _ in rd]
...
with sqlite3.connect('anki/collection.anki2') as conn:
    ...
    for eng, spa, tags in lines:
        ...
        n_sfld = "English: {{c1::"+eng+'}}<br />Spanish: {{c2::'+spa+'}}'
